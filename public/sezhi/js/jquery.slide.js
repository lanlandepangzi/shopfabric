...
(function($){
	$.fn.cxSlide=function(settings){
		...
		if(settings.btn&&!slide.numList.length){
			...
			slide.numList=$("<ul></ul>",{"class":"btns","html":_html}).appendTo(obj);
			slide.numBtns=slide.numList.find("li");
		};
		...
		// 事件：序号按钮
		if(settings.btn){
			slide.numList.delegate("li",settings.btnsEvents,function(){
